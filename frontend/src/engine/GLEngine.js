  ...
  loadScene(sceneJSON) {
    ...
    const ctx = {
      ...
      utils: {
        ...
        /**
         * Sample a Catmull-Rom spline defined by control points.
         * @param {number[][]} points - [[x,y], ...] sorted by x
         * @param {number} t - parameter (0–1) → interpolated y value
         */
        sampleCurve: (points, t) => {
          ...
          const u = (t - pts[i][0]) / dx;
          const u2 = u * u, u3 = u2 * u;
          const h00 = 2 * u3 - 3 * u2 + 1;
          const h10 = u3 - 2 * u2 + u;
          const h01 = -2 * u3 + 3 * u2;
          const h11 = u3 - u2;
          return h00 * pts[i][1] + h10 * m[i] * dx + h01 * pts[i + 1][1] + h11 * m[i + 1] * dx;
        },
